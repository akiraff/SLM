...
xlocs_arr = xcoor[6:16:2]  # 9 is not included
ylocs = int(ycoor[11])
resRow7 = CreateResRow(xlocs_arr, ylocs)

xlocs_arr = xcoor[5:17:2]  # 9 is not included
ylocs = int(ycoor[12])
resRow8 = CreateResRow(xlocs_arr, ylocs)

xlocs_arr = xcoor[6:16:2]  # 9 is not included
ylocs = int(ycoor[13])
resRow9 = CreateResRow(xlocs_arr, ylocs)

xlocs_arr = xcoor[7:15:2]  # 9 is not included
ylocs = int(ycoor[14])
resRow10 = CreateResRow(xlocs_arr, ylocs)

xlocs_arr = xcoor[8:14:2]  # 9 is not included
ylocs = int(ycoor[15])
resRow11 = CreateResRow(xlocs_arr, ylocs)

resin_arr = np.concatenate((resRow1, resRow2, resRow3, resRow4, resRow5, resRow6, resRow7, resRow8, resRow9, resRow10, resRow11), axis=0)

# Create resveroir outside the ring
xlocs = int(xcoor[-1] - round(spacing*1.5))
ylocs_arr = ycoor[[6,14]]
#print("ycoor = :", ycoor)
#print("ycoor site = :", ylocs_arr)
resCol1 = CreateResCol(xlocs, ylocs_arr)
xlocs = int(xcoor[-1] - round(spacing*1.5 + spacing*1.2))
ylocs_arr = ycoor[7:15:2]
...
xlocs_arr = xcoor[[5,15]]
ylocs = int(ycoor[0] + round(spacing*1.5))
resRow1 = CreateResRow(xlocs_arr, ylocs)
xlocs_arr = xcoor[[6,14]]
...
resout_arr = np.concatenate((resCol1, resCol2, resCol3, resCol4, resCol5, resCol6, resRow1, resRow2, resRow3, resRow4, resRow5, resRow6), axis=0)
print(resout_arr)
