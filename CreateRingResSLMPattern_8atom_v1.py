...
num_atom = 8
resRow2 = np.zeros([num_atom, 2])
resRow3 = np.zeros([num_atom, 2])
for j in range(num_atom):
    xlocs2 = xcoor[1]
    ylocs2 = int(ycoor[-1] - round(spacing*1.5+ spacing*1.1*j))
    resRow2[j,:] = [xlocs2, ylocs2]
    xlocs3 = xcoor[3]
    ylocs3 = ylocs2
    resRow3[j,:] = [xlocs3, ylocs3]



resout_arr = np.concatenate((resCol, resRow1, resRow2, resRow3), axis=0)
print(resout_arr)
...
target_arr = np.concatenate((ringcoor_arr,  resout_arr),axis=0)
